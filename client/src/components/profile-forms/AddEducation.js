...
const AddEducation = ({ addEducation, history }) => {
    ...
    const [toDateDisabeled, toggleToDate] = useState(false);    

    const { degree, school, fieldofstudy, to, from, current, description } = formData;
            ...
            <form className="form" onSubmit={e => {
                ...
                <p><input type="checkbox" name="current" checked={current} value={current} 
                    onChange= {e => {
                        ...
                        toggleToDate(!toDateDisabeled);
                    }} /> {' '}Current School</p>
                </div>
                <div className="form-group">
                <h4>To Date</h4>
                <input type="date" name="to" value={to} onChange={e => onChange(e)} disabled={toDateDisabeled ? 'disabled' : ''} />
                </div>
                <div className="form-group">
                <textarea
                    name="description"
